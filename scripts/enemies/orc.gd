...
## Orc enemy wielding an axe.
## Uses a custom 5x6 sprite sheet (orc.png) for top-down animations.
...
# --- Constants ---
const SPRITE_PATH: String = "res://assets/rpg/sprites/enemies/orc.png"

# Animation frame counts (using 4x4 sheet)
const IDLE_FRAMES: int = 1
...
func _ready() -> void:
	...
	# Disable hitbox shape initially (only enabled during active attack frame)
...
func _process_attack(delta: float) -> void:
	...
	# Enable hitbox on frame 2 and 3 of the attack
	if current_anim_frame == 2 or current_anim_frame == 3:
...
func _enter_hurt(from_position: Vector2) -> void:
	...
	velocity = knockback_dir * speed * 2.5
	move_and_slide()
...
func _enter_death() -> void:
	...
	# Set death animation (using hurt row 3 for death too)
	_set_anim(3, DEATH_FRAMES)
...
func _update_facing_from_velocity(dir: Vector2) -> void:
	...
	# If walk/chase state, update walk animation row
